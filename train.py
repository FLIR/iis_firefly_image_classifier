...
p.add_argument('--project_dir', type=str, default='./project_dir/',
               help='Directory where checkpoints and event logs are written to.')

p.add_argument('--project_name', type=str, default=None,
               help='Must supply project name examples: flower_classifier, component_classifier')

p.add_argument('--num_clones', type=int, default=1,
               help='Number of model clones to deploy. Note For historical reasons loss from all clones averaged out '
                    'and learning rate decay happen per clone epochs')

p.add_argument('--clone_on_cpu', type=bool, default=False,
               help='Use CPUs to deploy clones.')

p.add_argument('--worker_replicas', type=int, default=1,
               help='Number of worker replicas.')

p.add_argument('--num_ps_tasks', type=int, default=0,
               help='The number of parameter servers. If the value is 0, then the parameters are handled locally by '
                    'the worker.')

p.add_argument('--num_readers', type=int, default=4,
               help='The number of parallel readers that read data from the dataset.')

p.add_argument('--num_preprocessing_threads', type=int, default=4,
...
p.add_argument('--save_summaries_secs', type=int, default=20,
               help='The frequency with which summaries are saved, in seconds.')

p.add_argument('--save_interval_secs', type=int, default=10,
               help='The frequency with which the model is saved, in seconds.')

p.add_argument('--task', type=int, default=0,
               help='Task id of the replica running the training.')

p.add_argument('--verbose_placement', type=bool, default=False,
               help='Shows detailed information about device placement.')

p.add_argument('--hard_placement', type=bool, default=False,
               help='Uses hard constraints for device placement on tensorflow sessions.')

p.add_argument('--fixed_memory', type=bool, default=False,
               help='Allocates the entire memory at once.')
...
p.add_argument('--optimizer', type=str, default='adam',
               help='The name of the optimizer, one of "adadelta", "adagrad", "adam", "ftrl", "momentum", "sgd" or '
                    '"rmsprop".')

p.add_argument('--adadelta_rho', type=float, default=0.95,
               help='The decay rate for adadelta.')

p.add_argument('--adagrad_initial_accumulator_value', type=float, default=0.1,
...
p.add_argument('--opt_epsilon', type=float, default=1e-08,
               help='Epsilon term for the optimizer.')

p.add_argument('--ftrl_learning_rate_power', type=float, default=-0.5,
               help='The learning rate power.')

p.add_argument('--ftrl_initial_accumulator_value', type=float, default=0.1,
               help='Starting value for the FTRL accumulators.')

p.add_argument('--ftrl_l1', type=float, default=0.0,
               help='The FTRL l1 regularization strength.')

p.add_argument('--ftrl_l2', type=float, default=0.0,
               help='The FTRL l2 regularization strength.')

p.add_argument('--momentum', type=float, default=0.9,
               help='The momentum for the MomentumOptimizer and RMSPropOptimizer.')

p.add_argument('--rmsprop_momentum', type=float, default=0.9,
               help='Momentum.')

p.add_argument('--rmsprop_decay', type=float, default=0.9,
               help='Decay term for RMSProp.')

p.add_argument('--quantize_delay', type=int, default=-1,
               help='Number of steps to start quantized training. Set to -1 would disable quantized training.')

#######################
# Learning Rate Flags #
#######################

p.add_argument('--learning_rate_decay_type', type=str, default='exponential',
               help='Specifies how the learning rate is decayed. One of "fixed", "exponential", or "polynomial"')

p.add_argument('--end_learning_rate', type=float, default=0.0001,
               help='The minimal end learning rate used by a polynomial decay learning rate.')

p.add_argument('--label_smoothing', type=float, default=0.0,
               help='The amount of label smoothing.')

p.add_argument('--learning_rate_decay_factor', type=float, default=0.94,
               help='Learning rate decay factor.')

p.add_argument('--num_epochs_per_decay', type=float, default=2.0,
               help='Number of epochs after which learning rate decays. Note: this flag counts epochs per clone but '
                    'aggregates per sync replicas. So 1.0 means that each clone will go over full epoch individually, '
                    'but replicas will go once across all replicas.')

p.add_argument('--sync_replicas', type=bool, default=False,
               help='Whether or not to synchronize the replicas during training.')

p.add_argument('--replicas_to_aggregate', type=int, default=1,
               help='The Number of gradients to collect before updating params.')

p.add_argument('--moving_average_decay', type=float, default=None,
...
p.add_argument('--image_dir', type=str, default=None,
               help='The directory where the input images are saved.')

p.add_argument('--dataset_name', type=str, default='imagenet',
               help='The name of the dataset to load.')

p.add_argument('--dataset_split_name', type=str, default='train',
...
p.add_argument('--train_percentage', type=int, default=80,
               help='What percentage of images to use as a train set.')

p.add_argument('--validation_percentage', type=int, default=10,
               help='What percentage of images to use as a validation set.')

p.add_argument('--test_percentage', type=int, default=10,
               help='What percentage of images to use as a test set.')

p.add_argument('--labels_offset', type=int, default=0,
               help='An offset for the labels in the dataset. This flag is primarily used to evaluate the VGG and '
                    'ResNet architectures which do not use a background class for the ImageNet dataset.')

p.add_argument('--model_name', type=str, default='mobilenet_v1',
               help='The name of the architecture to train.')

p.add_argument('--preprocessing_name', type=str, default='custom_1_preprocessing_pipline',
               help='The name of the preprocessing to use. If left as `None`, then the model_name flag is used.')

p.add_argument('--batch_size', type=int, default=16,
               help='The number of samples in each batch.')

p.add_argument('--train_image_size', type=int, default=224,
               help='Train image size')

p.add_argument('--max_number_of_steps', type=int, default=50000,
               help='The maximum number of training steps.')

p.add_argument('--use_grayscale', type=bool, default=False,
               help='Whether to convert input images to grayscale.')

p.add_argument('--imbalance_correction', type=bool, default=False,
               help='apply class weight to loss function .')

#####################
# Fine-Tuning Flags #
#####################

p.add_argument('--feature_extraction', type=bool, default=False,
               help='Whether or not to synchronize the replicas during training.')
...
p.add_argument('--checkpoint_exclude_scopes', type=str, default=None,
               help='Comma-separated list of scopes of variables to exclude when restoring from a checkpoint. By '
                    'default, only the Logits layer is excluded')

p.add_argument('--trainable_scopes', type=str, default=None,
               help='Comma-separated list of scopes to filter the set of variables to train. By default, only the '
                    'Logits layer is trained. None would train all the variables.')
...
p.add_argument('--ignore_missing_vars', type=bool, default=True,
               help='When restoring a checkpoint would ignore missing variables.')

# this argument was added for mobilenet_v1.py
p.add_argument('--final_endpoint', type=str, default=None,
               help='Specifies the endpoint to construct the network up to. By default, None would be the last layer '
                    'before Logits.')
...
p.add_argument('--experiment_tag', type=str, default='',
               help='Internal tag for experiment')

p.add_argument('--experiment_name', type=str, default=None,
               help='If None a new experiment folder is created. Naming convention experiment_number')
...
p.add_argument('--add_image_summaries', type=bool, default=True,
               help='Enable image summaries.')

p.add_argument('--apply_image_augmentation', type=bool, default=True,
               help='Enable random image augmentation during preprocessing for training.')

p.add_argument('--random_image_crop', type=bool, default=True,
               help='Enable random cropping of images. Only Enabled if apply_image_augmentation flag is also enabled')

p.add_argument('--min_object_covered', type=float, default=0.9,
               help='The remaining cropped image must contain at least this fraction of the whole image. Only Enabled '
                    'if apply_image_augmentation flag is also enabled')

p.add_argument('--random_image_rotation', type=bool, default=True,
               help='Enable random image rotation counter-clockwise by 90, 180, 270, or 360 degrees. Only Enabled if '
                    'apply_image_augmentation flag is also enabled')

p.add_argument('--random_image_flip', type=bool, default=False,
               help='Enable random image flip (horizontally). Only Enabled if apply_image_augmentation flag is also '
                    'enabled')

p.add_argument('--roi', type=str, default=None,
               help='Specifies the coordinates of an ROI for cropping the input images.Expects four integers in the '
                    'order of roi_y_min, roi_x_min, roi_height, roi_width, image_height, image_width. Only applicable '
                    'to mobilenet_preprocessing pipeline ')

p.add_argument("--eval_image_size", type=int, default=None,
               help='Eval image size')

p.add_argument("--eval_interval_secs", type=int, default=20,
               help='The frequency with which the model is evaluated')

p.add_argument("--eval_timeout_secs", type=int, default=None,
               help='The maximum amount of time to wait between checkpoints. If left as None, then the process will '
                    'wait for double the eval_interval_secs.')

p.add_argument("--quantize", type=bool, default=False,
               help='whether to use quantized graph or not.')

p.add_argument("--max_num_batches", type=int, default=None,
               help='Max number of batches to evaluate by default use all.')
...
class TrainClassifier:
    ...
    def configure_learning_rate(self, num_samples_per_epoch, global_step):
      ...
      """
        ...
        decay_steps = int(num_samples_per_epoch * self.FLAGS.num_epochs_per_decay /
                          self.FLAGS.batch_size)

        if self.FLAGS.sync_replicas:
            decay_steps /= self.FLAGS.replicas_to_aggregate

        if self.FLAGS.learning_rate_decay_type == 'exponential':
            return tf.train.exponential_decay(self.FLAGS.learning_rate,
                                              global_step,
                                              decay_steps,
            ...
            return tf.constant(self.FLAGS.learning_rate, name='fixed_learning_rate')
        elif self.FLAGS.learning_rate_decay_type == 'polynomial':
            return tf.train.polynomial_decay(self.FLAGS.learning_rate,
                                             global_step,
                                             decay_steps,
    ...
    def _get_init_fn(self):
      ...
      """
        # print('####################0')
        checkpoint_path = ""
        if not self.FLAGS.checkpoint_path:
            # download imagenet pre-trained model weights
            if self.FLAGS.model_name == 'inception_v1':
            ...
            elif self.FLAGS.model_name == 'mobilenet_v1':
            ...
            elif self.FLAGS.model_name == 'mobilenet_v1_075':
                checkpoint_path = './imagenet_checkpoints/mobilenet_v1_0.75_224/mobilenet_v1_0.75_224.ckpt'
                if not os.path.isfile(checkpoint_path):
                    url = os.path.join(
                        'http://download.tensorflow.org/models/mobilenet_v1_2018_02_22/mobilenet_v1_0.75_224.tgz')
                    self.download_and_extract_file(checkpoint_path, url)
            elif self.FLAGS.model_name == 'mobilenet_v1_050':
                checkpoint_path = './imagenet_checkpoints/mobilenet_v1_0.5_224/mobilenet_v1_0.5_224.ckpt'
                if not os.path.isfile(checkpoint_path):
                    url = os.path.join(
                        'http://download.tensorflow.org/models/mobilenet_v1_2018_02_22/mobilenet_v1_0.5_224.tgz')
                    self.download_and_extract_file(checkpoint_path, url)
            elif self.FLAGS.model_name == 'mobilenet_v1_025':
                checkpoint_path = './imagenet_checkpoints/mobilenet_v1_0.25_224/mobilenet_v1_0.25_224.ckpt'
                if not os.path.isfile(checkpoint_path):
                    url = os.path.join(
                        'http://download.tensorflow.org/models/mobilenet_v1_2018_02_22/mobilenet_v1_0.25_224.tgz')
                    self.download_and_extract_file(checkpoint_path, url)
        else:
        ...
        if self.FLAGS.checkpoint_exclude_scopes:
            ...
            model_variables = self.model_name_to_variables.get(self.FLAGS.model_name)
        ...
        if self.FLAGS.feature_extraction:
        ...
        return self.slim.assign_from_checkpoint_fn(
            checkpoint_path,
            variables_to_restore,
            ignore_missing_vars=self.FLAGS.ignore_missing_vars)
    ...
    def _get_variables_to_train(self):
        ...
        """
        scopes = None
        if self.FLAGS.trainable_scopes is None or self.FLAGS.num_of_trainable_layers == 1:
            print('model name ################', self.FLAGS.model_name)
            if self.FLAGS.model_name.startswith('inception_v1'):
                scopes = ['InceptionV1/Logits', 'BatchNorm']
            elif self.FLAGS.model_name.startswith('mobilenet_v1'):
                scopes = ['MobilenetV1/Logits', 'BatchNorm']
        elif self.FLAGS.num_of_trainable_layers == 2:
            if self.FLAGS.model_name.startswith('inception_v1'):
                scopes = ['InceptionV1/Logits', 'InceptionV1/Mixed_5c']
            elif self.FLAGS.model_name.startswith('mobilenet_v1'):
                scopes = ['MobilenetV1/Logits', 'MobilenetV1/Conv2d_13']
        elif self.FLAGS.num_of_trainable_layers == 3:
            if self.FLAGS.model_name.startswith('inception_v1'):
        ...
        elif self.FLAGS.num_of_trainable_layers == 4:
            if self.FLAGS.model_name.startswith('inception_v1'):
                scopes = ['InceptionV1/Logits', 'BatchNorm', 'InceptionV1/Mixed_5c', 'InceptionV1/Mixed_5b',
                          'InceptionV1/Mixed_4f']
            elif self.FLAGS.model_name.startswith('mobilenet_v1'):
                scopes = ['MobilenetV1/Logits', 'BatchNorm', 'MobilenetV1/Conv2d_13', 'MobilenetV1/Conv2d_12',
                          'MobilenetV1/Conv2d_11']
        else:
            scopes = [scope.strip() for scope in self.FLAGS.trainable_scopes.split(',')]
    ...
    def check_create_project_folders(self):
        ...
        if not self.FLAGS.dataset_name:
            raise ValueError('You must supply a dataset name with --dataset_name')
        if self.FLAGS.model_name not in self.model_name_to_variables:
            raise ValueError(
                'Model name not supported name please select one of the following model architecture: mobilenet_v1, '
                'mobilenet_v1_075, mobilenet_v1_050, mobilenet_v1_025, inception_v1')
        ...
        self.project_dir = os.path.join(self.FLAGS.project_dir, self.FLAGS.project_name)
        if not self.FLAGS.experiment_name:
            ...
            self.experiment_dir = os.path.join(os.path.join(self.project_dir, 'experiments'),
                                               self.FLAGS.experiment_name)
            if not os.path.exists(self.experiment_dir):
                raise ValueError('Experiment directory {} does not exist.'.format(self.experiment_dir))

        self.train_dir = os.path.join(self.experiment_dir, self.FLAGS.dataset_split_name)
    ...
    def create_dataset(self):
        ...
        if self.FLAGS.image_dir:
            self.dataset_dir = convert_dataset.convert_img_to_tfrecord(self.project_dir,
                                                                       ...
                                                                       self.FLAGS.train_percentage,
                                                                       self.FLAGS.validation_percentage,
                                                                       self.FLAGS.test_percentage,
                                                                       self.FLAGS.train_image_size,
                                                                       self.FLAGS.train_image_size)
        else:
            if os.path.isdir(self.FLAGS.dataset_dir):
                self.dataset_dir = os.path.join(self.FLAGS.dataset_dir, self.FLAGS.dataset_name)
            else:
                self.dataset_dir = os.path.join(os.path.join(self.project_dir, 'datasets'), self.FLAGS.dataset_name)
    ...
    def prepare_train(self):
        ...
        with self.graph.as_default():
            ...
            deploy_config = model_deploy.DeploymentConfig(
                num_clones=self.FLAGS.num_clones,
                clone_on_cpu=self.FLAGS.clone_on_cpu,
                replica_id=self.FLAGS.task,
                num_replicas=self.FLAGS.worker_replicas,
                num_ps_tasks=self.FLAGS.num_ps_tasks)
            ...
            dataset = dataset_factory.get_dataset(
                self.FLAGS.dataset_name, 'train', self.dataset_dir)
            ...
            network_fn = nets_factory.get_network_fn(
                ...
                num_classes=(dataset.num_classes - self.FLAGS.labels_offset),
                weight_decay=self.FLAGS.weight_decay,
                is_training=not self.FLAGS.feature_extraction,
                final_endpoint=self.FLAGS.final_endpoint)

            #####################################
            # Select the preprocessing function #
            #####################################
            preprocessing_name = self.FLAGS.preprocessing_name or self.FLAGS.model_name
            image_preprocessing_fn = preprocessing_factory.get_preprocessing(
                preprocessing_name,
                is_training=self.FLAGS.apply_image_augmentation,
                use_grayscale=self.FLAGS.use_grayscale)
            ...
            with tf.device(deploy_config.inputs_device()):
                ...
                label -= self.FLAGS.labels_offset

                train_image_size = self.FLAGS.train_image_size or network_fn.default_image_size

                image = image_preprocessing_fn(image,
                                               train_image_size, train_image_size,
                                               add_image_summaries=self.FLAGS.add_image_summaries,
                                               crop_image=self.FLAGS.random_image_crop,
                                               min_object_covered=self.FLAGS.min_object_covered,
                                               rotate_image=self.FLAGS.random_image_rotation,
                                               random_flip=self.FLAGS.random_image_flip,
                                               roi=self.FLAGS.roi)

                images, labels = tf.train.batch(
                    [image, label],
                    batch_size=self.FLAGS.batch_size,
                    num_threads=self.FLAGS.num_preprocessing_threads,
                    capacity=5 * self.FLAGS.batch_size)
                labels = self.slim.one_hot_encoding(
                    labels, dataset.num_classes - self.FLAGS.labels_offset)
            ...
            # Define the model #
            ####################
            def clone_fn(batch_queue_):
                """Allows data parallelism by creating multiple clones of network_fn."""
                images_, labels_ = batch_queue_.dequeue()
                logits, end_points_ = network_fn(images_)

                #############################
                # Specify the loss function #
                #############################
                if self.FLAGS.imbalance_correction:
                    # specify some class weightings
                    class_weights = dataset.sorted_class_weights
                    # deduce weights for batch samples based on their true label
                    weights = tf.reduce_sum(tf.multiply(labels_, class_weights), 1)

                    self.slim.losses.softmax_cross_entropy(
                        logits, labels_, label_smoothing=self.FLAGS.label_smoothing, weights=weights)
                else:
                    if 'AuxLogits' in end_points_:
                        self.slim.losses.softmax_cross_entropy(
                            end_points_['AuxLogits'], labels_,
                            label_smoothing=self.FLAGS.label_smoothing, weights=0.4,
                            scope='aux_loss')
                    else:
                        self.slim.losses.softmax_cross_entropy(
                            logits, labels_, label_smoothing=self.FLAGS.label_smoothing, weights=1.0)
                #############################
                #  Calculation of metrics   #
                #############################
                accuracy_, accuracy_op_ = tf.metrics.accuracy(tf.argmax(labels_, 1), tf.argmax(logits, 1))
                precision_, precision_op_ = tf.metrics.average_precision_at_k(tf.argmax(labels_, 1), logits, 1)

                with tf.device('/device:CPU:0'):
                    for class_id_ in range(dataset.num_classes):
                        precision_at_k_, precision_at_k_op_ = tf.metrics.precision_at_k(tf.argmax(labels_, 1),
                                                                                        logits,
                                                                                        k=1,
                                                                                        class_id=class_id_)
                        recall_at_k_, recall_at_k_op_ = tf.metrics.recall_at_k(tf.argmax(labels_, 1), logits, k=1,
                                                                               class_id=class_id_)
                        tf.add_to_collection('precision_at_{}'.format(class_id_), precision_at_k_)
                        tf.add_to_collection('precision_at_{}_op'.format(class_id_), precision_at_k_op_)
                        tf.add_to_collection('recall_at_{}'.format(class_id_), recall_at_k_)
                        tf.add_to_collection('recall_at_{}_op'.format(class_id_), recall_at_k_op_)

                tf.add_to_collection('accuracy', accuracy_)
                tf.add_to_collection('accuracy_op', accuracy_op_)
                tf.add_to_collection('precision', precision_)
                tf.add_to_collection('precision_op', precision_op_)
            ...
            if self.FLAGS.quantize_delay >= 0:
                contrib_quantize.create_training_graph(quant_delay=self.FLAGS.quantize_delay)
            ...
            if self.FLAGS.sync_replicas:
                ...
                optimizer = tf.train.SyncReplicasOptimizer(
                    opt=optimizer,
                    replicas_to_aggregate=self.FLAGS.replicas_to_aggregate,
                    total_num_replicas=self.FLAGS.worker_replicas,
                    variable_averages=variable_averages,
                    variables_to_average=moving_average_variables)
            elif self.FLAGS.moving_average_decay:
            ...
            self.session_config = tf.ConfigProto(
                log_device_placement=self.FLAGS.verbose_placement,
                allow_soft_placement=not self.FLAGS.hard_placement)
            if not self.FLAGS.fixed_memory:
    ...
    def prepare_eval(self):
        ...
        with self.eval_graph.as_default():
            ...
            ######################
            dataset = dataset_factory.get_dataset(
                self.FLAGS.dataset_name, 'validation', self.dataset_dir)
            ...
            provider = self.slim.dataset_data_provider.DatasetDataProvider(
                ...
                shuffle=False,
                num_epochs=None,
                common_queue_capacity=2 * self.FLAGS.batch_size,
                common_queue_min=self.FLAGS.batch_size)
            [image, label] = provider.get(['image', 'label'])
            label -= self.FLAGS.labels_offset
            ...
            eval_image_size = self.FLAGS.eval_image_size or network_fn.default_image_size
            ...
            if self.FLAGS.quantize:
                contrib_quantize.create_eval_graph()

            if self.FLAGS.moving_average_decay:
                variable_averages = tf.train.ExponentialMovingAverage(
                    self.FLAGS.moving_average_decay, tf_global_step)
            ...
            for class_id in range(dataset.num_classes):
                precision_at_k, precision_at_k_op = tf.metrics.precision_at_k(tf.squeeze(labels), logits, k=1,
                                                                              class_id=class_id)
            ...
            if self.FLAGS.max_num_batches:
                self.num_batches = self.FLAGS.max_num_batches
            else:
                # This ensures that we make a single pass over all of the data.
                self.num_batches = math.ceil(dataset.num_samples / float(self.FLAGS.batch_size))

            # if checkpoint_path flag is none, look for checkpoint in experiment train directory
            print('################ experiment_dir', self.experiment_dir, self.FLAGS.checkpoint_path)
            # if self.FLAGS.checkpoint_path is None:
            ...
            # if not self.FLAGS.eval_timeout_secs:
            #     eval_timeout_secs = self.FLAGS.eval_interval_secs * 2
            # else:
            #     eval_timeout_secs = self.FLAGS.eval_timeout_secs
            #
            # with self.eval_graph.as_default():
    ...
    def train(self):
          ...
          """
            ...
            if 'should_log' in train_step_kwargs:
                if sess.run(train_step_kwargs['should_log']):
                    print('global step {:d}: loss = {:1.4f} ({:.3f} sec/step)'.format(np_global_step, total_loss,
                                                                                      time_elapsed))
        ...
        with self.graph.as_default():
            self.slim.learning.train(
                ...
                master=self.FLAGS.master,
                is_chief=(self.FLAGS.task == 0),
                init_fn=self._get_init_fn(),
                summary_op=self.summary_op,
                number_of_steps=self.FLAGS.max_number_of_steps,
                log_every_n_steps=self.FLAGS.log_every_n_steps,
                save_summaries_secs=self.FLAGS.save_summaries_secs,
                save_interval_secs=self.FLAGS.save_interval_secs,
                sync_optimizer=self.optimizer if self.FLAGS.sync_replicas else None,
        ...
        output_file = os.path.join(self.train_dir, self.FLAGS.model_name + '_graph.pb')
        export_inference_graph(self.FLAGS.dataset_name, self.dataset_dir, self.FLAGS.model_name,
                               self.FLAGS.labels_offset, is_training, self.FLAGS.final_endpoint,
                               self.FLAGS.train_image_size, self.FLAGS.use_grayscale, is_video_model, batch_size,
                               num_frames, quantize, write_text_graphdef, output_file)
