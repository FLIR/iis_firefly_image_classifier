...
p.add_argument('--save_summaries_secs', type=int, default=5, help='The frequency with which summaries are saved, in seconds.')

p.add_argument('--save_interval_secs', type=int, default=5, help='The frequency with which the model is saved, in seconds.')
...
class TrainClassifier:
    ...
    def check_create_project_folders(self):
        ...
        if not self.FLAGS.model_name in self.model_name_to_variables:
        ...
        if not self.FLAGS.experiment_name:
          # list only directories that are names experiment_
            ...
            self.experiment_dir = os.path.join(os.path.join(self.project_dir, 'experiments'), self.FLAGS.experiment_name)
    ...
    def prepare_train(self):
        ...
        #
        # # set and check project_dir and experiment_dir.
        # project_dir = os.path.join(self.FLAGS.project_dir, self.FLAGS.project_name)
        # if not self.FLAGS.experiment_name:
        #   # list only directories that are names experiment_
        #     self.experiment_dir = dataset_utils.create_new_experiment_dir(project_dir)
        # else:
        #     self.experiment_dir = os.path.join(os.path.join(project_dir, 'experiments'), self.FLAGS.experiment_name)
        #     if not os.path.exists(self.experiment_dir):
        #         raise ValueError('Experiment directory {} does not exist.'.format(self.experiment_dir))
        #
        #
        ...
        #         self.dataset_dir = os.path.join(self.FLAGS.dataset_dir, self.FLAGS.dataset_name)
        #     else:
        #         self.dataset_dir = os.path.join(os.path.join(project_dir, 'datasets'), self.FLAGS.dataset_name)
        # if not os.path.isdir(self.dataset_dir):
        #   raise ValueError('Can not find tfrecord dataset directory {}'. format(self.dataset_dir))
          ...
          def clone_fn(batch_queue):
            ...
            if self.FLAGS.imbalance_correction:
                ...
                self.slim.losses.softmax_cross_entropy(
                          logits, labels, label_smoothing=self.FLAGS.label_smoothing, weights=weights)
            else:
                if 'AuxLogits' in end_points:
                  ...
                  self.slim.losses.softmax_cross_entropy(
                          logits, labels, label_smoothing=self.FLAGS.label_smoothing, weights=1.0)
            #############################
    ...
    def evaluate(self):
        ...
        elif os.path.isfile(FLAGS.checkpoint_path):
            checkpoint_path = FLAGS.checkpoint_path
        else:
            checkpoint_path = os.path.join(self.experiment_dir, 'train')
            print('###########55', checkpoint_path)
            checkpoint_path = tf.train.latest_checkpoint(checkpoint_path)
            print('###########555', checkpoint_path)
        ...
        if not self.FLAGS.eval_timeout_secs:
            ...
            eval_timeout_secs = self.FLAGS.eval_timeout_secs
        print('###############5555', checkpoint_path)

        with self.eval_graph.as_default():
            # Evaluate once:
            # print('############', type(self.update_op))
            self.slim.evaluation.evaluate_once(
                master=self.FLAGS.master,
    ...
    def train(self):
      ...
      export_inference_graph(self.FLAGS.dataset_name, self.dataset_dir,  self.FLAGS.model_name, self.FLAGS.labels_offset, is_training, self.FLAGS.final_endpoint, self.FLAGS.train_image_size, self.FLAGS.use_grayscale, is_video_model, batch_size, num_frames, quantize, write_text_graphdef, output_file)
